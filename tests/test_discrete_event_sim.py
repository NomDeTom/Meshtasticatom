...
class TestDiscreteEventSim(unittest.TestCase):
    ...
    def test_discrete_sim_ten_nodes(self):
        ...
        self.assertEqual(messageSeq, 182, "expected number of messages created")
        sent = results['sent']
        potentialReceivers = results['potentialReceivers']
        self.assertEqual(sent, 816, "expected number of packets sent")
        self.assertEqual(potentialReceivers, 7344, "expected number of potential receivers")

        nrCollisions = results['nrCollisions']
        self.assertEqual(nrCollisions, 210, "expected number of collisions")
        nrSensed = results['nrSensed']
        self.assertEqual(nrSensed, 2801, "expected number of packets sensed")

        nrReceived = results['nrReceived']
        self.assertEqual(nrReceived, 2587, "expected number of packets received")
        meanDelay = results['meanDelay']
        self.assertEqual(round(meanDelay, 2), 4512.33, "expected rounded delay average")
        txAirUtilizationRate = results['txAirUtilizationRate']
        self.assertEqual(round(txAirUtilizationRate * 100, 2), 3.09, "expected rounded average tx air utilization")

        nodeReach = results['nodeReach']
        self.assertEqual(round(nodeReach*100, 2), 81.93, "expected rounded percentage of nodes reached")

        usefulness = results['usefulness']
        self.assertEqual(round(usefulness*100, 2), 51.87, "expected rounded 'usefulness' percentage")

        delayDropped = results['delayDropped']
        self.assertEqual(delayDropped, 1135, "expected number of packets dropped")
