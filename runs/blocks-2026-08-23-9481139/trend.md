...
- SF-capacity: capacity=4: decode_failures 82
...
- DG-outage: burst-loss=0.3: decode_failures 36
...
- SF-capacity-window: capacity=16: misdecodes 28
...
- MS-size: nodes=40: decode_failures 4
- SF-capacity-local: capacity=4: decode_failures 82
...
- SF-hops-spread: hops-apart=5: decode_failures 35
- SF-place-spread: place=spread: decode_failures 24
- SF-place-spread: place=random-clients: decode_failures 6
- RF-bw500: preset=SHORT_TURBO: decode_failures 9
- SF-catchup: catch-up-hours=: misdecodes 14
- SF-catchup: catch-up-hours=02-06: decode_failures 44
- SF-catchup: catch-up-hours=00-08: decode_failures 42
- TH-congestion: no-congestion-scaling=True: queue drops 12.6% of transmissions - airtime here is measured through a cap
...
- RF-preset: preset=LONG_MODERATE: decode_failures 4
- MS-topology: topology=clustered: decode_failures 18
- MS-topology: slower: 3.6 s per simulated hour against 1.67 over 2 prior run(s) - 2.1x, and a runtime regression is invisible to `timeout-minutes` until it fails a job outright
- TH-congestion-input: congestion-input=hotstore: decode_failures 13
- TH-congestion-input: congestion-input=truesize: decode_failures 6
- RT-favourites: faster: 0.999 s per simulated hour against 2.08 over 2 prior run(s) - 2.1x quicker, which is worth a look: a fragmented mesh or an arm that stopped being read both cost less to simulate
- MS-hopscale: nodes=250: decode_failures 22
- MS-hopscale: nodes=500: decode_failures 120
- DB-hotstore-stress: max-num-nodes=10: decode_failures 105
- DB-hotstore-stress: max-num-nodes=120: decode_failures 13
- DB-hotstore-stress: max-num-nodes=250: decode_failures 1
- MS-oversubscribed: nodes=250: decode_failures 13
- MS-oversubscribed: nodes=500: decode_failures 83
- MS-siting: siting-mix=local-typical: misdecodes 1
- LD-traceroute-small: traceroute-per-hour=0.0: queue drops 13.9% of transmissions - airtime here is measured through a cap
- LD-traceroute-small: traceroute-per-hour=0.0: decode_failures 69
- LD-traceroute-small: traceroute-per-hour=1.0: queue drops 20.8% of transmissions - airtime here is measured through a cap
- LD-traceroute-small: traceroute-per-hour=1.0: decode_failures 62
- DB-warm: warm-num-nodes=0: queue drops 13.9% of transmissions - airtime here is measured through a cap
- DB-warm: warm-num-nodes=0: decode_failures 69
- DB-warm: warm-num-nodes=25: queue drops 13.9% of transmissions - airtime here is measured through a cap
- DB-warm: warm-num-nodes=25: decode_failures 69
- DB-warm: warm-num-nodes=100: queue drops 13.9% of transmissions - airtime here is measured through a cap
- DB-warm: warm-num-nodes=100: decode_failures 69
- DB-warm: warm-num-nodes=2000: queue drops 13.9% of transmissions - airtime here is measured through a cap
- DB-warm: warm-num-nodes=2000: decode_failures 69
- LD-chatty-hops: broadcast-interval-s=300: decode_failures 27
- LD-chatty: broadcast-interval-s=300: decode_failures 22
- AD-nomute: faster: 1.6 s per simulated hour against 3.91 over 2 prior run(s) - 2.4x quicker, which is worth a look: a fragmented mesh or an arm that stopped being read both cost less to simulate
- AD-siting: siting-mix=local-typical: misdecodes 1
- AD-siting: faster: 1.25 s per simulated hour against 4.06 over 2 prior run(s) - 3.2x quicker, which is worth a look: a fragmented mesh or an arm that stopped being read both cost less to simulate
- RF-stretch-duct: duct-per-hour=0.0: decode_failures 13
- MS-stretch: stretch=1.25: decode_failures 23
- MS-stretch: stretch=1.5: decode_failures 13
- MS-stretch: stretch=2.0: decode_failures 4
- AD-worst: role-placement=degree: decode_failures 108
- AD-worst: role-placement=inverse: decode_failures 82
- AD-worst: slower: 23.7 s per simulated hour against 2.89 over 2 prior run(s) - 8.2x, and a runtime regression is invisible to `timeout-minutes` until it fails a job outright
...
| --- | --: | --: | --: | --: |
| `AD-worst` | 23.7 | 2.89 | 8.18x | 2 |
| `MS-topology` | 3.6 | 1.67 | 2.15x | 2 |
| `RF-stretch-duct` | 3.57 | 2.16 | 1.65x | 2 |
| `RF-txpower` | 2.57 | 1.61 | 1.59x | 2 |
| `DG-burst` | 5.02 | 7.53 | 0.67x | 2 |
| `AD-badrouters` | 1.7 | 2.57 | 0.66x | 2 |
| `SF-cadence` | 2.38 | 4.13 | 0.58x | 2 |
| `MS-roles` | 1.73 | 3.16 | 0.55x | 2 |
| `RF-eu-presets` | 2.03 | 3.71 | 0.55x | 2 |
| `SF-servers-allrouters` | 2.13 | 4.17 | 0.51x | 2 |
| `AD-flooding` | 2.52 | 4.95 | 0.51x | 2 |
| `MS-density` | 2.81 | 5.58 | 0.50x | 2 |
| `RT-favourites` | 0.999 | 2.08 | 0.48x | 2 |
| `AD-nomute` | 1.6 | 3.91 | 0.41x | 2 |
| `AD-siting` | 1.25 | 4.06 | 0.31x | 2 |
...
| --- | --- | --- | --- | --: | --- | --- | :-: | --: |
| `BL-control` | protocol | **held** | 0 → 0.946 | 0.946 | 0.804 → 0.824 | 1.1x bytes_on_air | up | 2 |
| `PR-protocol` | protocol | **held** | 0 → 0.944 | 0.944 | 0.797 → 0.808 | 1.2x bytes_on_air | up | 3 |
| `RF-txpower` | tx-power | **held** | 0.088 → 0.944 | 0.855 | 0.109 → 0.808 | 12x sr_airtime | down | 4 |
| `RF-pulse` | noise-pulse-interval-ms | **held** | 0.120 → 0.892 | 0.772 | 0.096 → 0.743 | 95x sr_airtime | down | 4 |
| `AD-siting` | siting-mix | **held** | 0.132 → 0.892 | 0.761 | 0.084 → 0.694 | 7.1x advert_bytes | down | 3 |
| `MS-stretch` | stretch | **text** | 0.133 → 0.808 | 0.674 | 0.133 → 0.808 | 3.4x sr_airtime | down | 4 |
| `MS-siting` | siting-mix | **text** | 0.387 → 0.960 | 0.573 | 0.387 → 0.960 | 3.2x sr_airtime | up | 4 |
| `RF-bw500` | preset | **held** | 0.398 → 0.942 | 0.544 | 0.196 → 0.730 | 2.4x advert_bytes | up | 3 |
| `MS-hopscale` | nodes | **text** | 0.330 → 0.808 | 0.477 | 0.330 → 0.808 | 12x sr_bytes | down | 4 |
| `RF-eu-presets` | preset | **text** | 0.351 → 0.808 | 0.456 | 0.351 → 0.808 | 2.8x sr_airtime | up | 4 |
| `RF-preset` | preset | **text** | 0.351 → 0.808 | 0.456 | 0.351 → 0.808 | 4x sr_airtime | up | 3 |
| `MS-oversubscribed` | nodes | **held** | 0.496 → 0.917 | 0.421 | 0.334 → 0.718 | 5x sr_bytes | down | 3 |
| `MS-density` | nodes | **text** | 0.598 → 0.952 | 0.354 | 0.598 → 0.952 | 4.4x advert_bytes | up | 5 |
| `LD-chatty` | broadcast-interval-s | **text** | 0.496 → 0.829 | 0.333 | 0.496 → 0.829 | 7.9x sr_airtime | down | 3 |
| `DG-outage` | burst-loss | **text** | 0.477 → 0.808 | 0.331 | 0.477 → 0.808 | 2.6x sr_bytes | down | 4 |
| `MS-topology` | topology | **text** | 0.597 → 0.924 | 0.327 | 0.597 → 0.924 | 3.1x sr_bytes | up | 4 |
| `LD-chatty-hops` | broadcast-interval-s | **text** | 0.554 → 0.879 | 0.326 | 0.554 → 0.879 | 7.8x sr_airtime | down | 3 |
| `DG-burst` | burst-loss | **text** | 0.500 → 0.808 | 0.307 | 0.500 → 0.808 | 2.5x sr_bytes | down | 4 |
| `RF-stretch-duct` | duct-per-hour | **text** | 0.391 → 0.683 | 0.293 | 0.391 → 0.683 | 1.9x sr_airtime | up | 2 |
| `DB-hotstore-stress` | max-num-nodes | **held** | 0.612 → 0.899 | 0.287 | 0.312 → 0.524 | 6x sr_airtime | up | 3 |
| `TH-congestion` | no-congestion-scaling | **text** | 0.652 → 0.920 | 0.268 | 0.652 → 0.920 | 4.6x sr_airtime | down | 2 |
| `SF-hops-flat` | hops-apart | **held** | 0.691 → 0.946 | 0.254 | 0.808 → 0.824 | 1.9x sr_bytes | down | 4 |
| `SF-hops-spread` | hops-apart | **held** | 0.691 → 0.946 | 0.254 | 0.808 → 0.824 | 1.9x sr_bytes | down | 5 |
| `RT-hoplimit` | hop-limit | **text** | 0.645 → 0.892 | 0.246 | 0.645 → 0.892 | 2.1x sr_bytes | up | 4 |
| `RT-hopspread` | hop-limit | **text** | 0.645 → 0.870 | 0.225 | 0.645 → 0.870 | 2x sr_bytes | up | 3 |
| `SF-place-flat` | place | **held** | 0.754 → 0.951 | 0.197 | 0.808 → 0.830 | 3.1x sr_bytes | up | 6 |
| `SF-place-spread` | place | **held** | 0.754 → 0.951 | 0.197 | 0.808 → 0.830 | 3.1x sr_bytes | up | 6 |
| `RT-spread` | hop-spread | **text** | 0.645 → 0.808 | 0.162 | 0.645 → 0.808 | 1.9x sr_bytes | up | 2 |
| `RF-noise` | noise-profile | **text** | 0.648 → 0.809 | 0.161 | 0.648 → 0.809 | 1.4x sr_bytes | down | 4 |
| `AD-flooding` | role-mix | **text** | 0.694 → 0.851 | 0.157 | 0.694 → 0.851 | 2.5x bytes_on_air | up | 2 |
| `AD-nomute` | role-mix | **text** | 0.694 → 0.851 | 0.157 | 0.694 → 0.851 | 2.5x bytes_on_air | up | 3 |
| `DG-loss` | extra-loss | **text** | 0.663 → 0.808 | 0.144 | 0.663 → 0.808 | 1.8x sr_bytes | down | 4 |
| `MS-size` | nodes | **text** | 0.664 → 0.808 | 0.144 | 0.664 → 0.808 | 4.3x advert_bytes | down | 5 |
| `AD-amplify-worst` | amplify-worst | **text** | 0.808 → 0.938 | 0.130 | 0.808 → 0.938 | 1.2x sr_airtime | up | 3 |
| `DB-hotstore` | max-num-nodes | **text** | 0.733 → 0.856 | 0.122 | 0.733 → 0.856 | 2.1x sr_airtime | up | 4 |
| `MS-roles` | role-mix | **text** | 0.694 → 0.809 | 0.114 | 0.694 → 0.809 | 1.2x bytes_on_air | down | 2 |
| `DB-platform` | platform-mix | **text** | 0.743 → 0.856 | 0.113 | 0.743 → 0.856 | 2.1x sr_airtime | down | 3 |
| `LD-interval` | broadcast-interval-s | **text** | 0.755 → 0.866 | 0.111 | 0.755 → 0.866 | 5.3x sr_airtime | up | 4 |
| `SC-signing` | signature-policy | **text** | 0.703 → 0.808 | 0.104 | 0.703 → 0.808 | 1.2x sr_airtime | down | 3 |
| `FW-mixed-26` | legacy-fraction | **text** | 0.768 → 0.869 | 0.101 | 0.768 → 0.869 | 2.1x bytes_on_air | down | 4 |
| `AD-amplifiers` | amplifier-mix | **text** | 0.808 → 0.906 | 0.098 | 0.808 → 0.906 | 1.3x sr_bytes | up | 3 |
| `FW-mixed` | legacy-fraction | **text** | 0.759 → 0.855 | 0.095 | 0.759 → 0.855 | 2x bytes_on_air | down | 4 |
| `MS-roles-fav` | role-mix | **text** | 0.753 → 0.842 | 0.089 | 0.753 → 0.842 | 1.1x sr_airtime | down | 2 |
| `RF-duct` | duct-per-hour | **text** | 0.808 → 0.891 | 0.084 | 0.808 → 0.891 | 1.3x bytes_on_air | up | 3 |
| `LD-traceroute-small` | traceroute-per-hour | **held** | 0.704 → 0.772 | 0.068 | 0.586 → 0.645 | 1.3x sr_airtime | down | 2 |
| `FW-signing-cost` | profile-flag | **text** | 0.808 → 0.858 | 0.050 | 0.808 → 0.858 | 3.3x bytes_on_air | down | 2 |
| `LD-traceroute` | traceroute-per-hour | **text** | 0.765 → 0.808 | 0.042 | 0.765 → 0.808 | 1.4x sr_airtime | down | 4 |
| `SF-cadence` | trigger | **held** | 0.902 → 0.944 | 0.042 | 0.778 → 0.808 | 13x advert_bytes | down | 4 |
| `RT-favourites` | favourite-routers | **text** | 0.800 → 0.839 | 0.039 | 0.800 → 0.839 | 1.1x sr_bytes | up | 2 |
| `RT-rebroadcast` | rebroadcast-mode | **held** | 0.910 → 0.944 | 0.033 | 0.808 → 0.810 | 15x sr_airtime | down | 3 |
| `SF-provide-transport` | provide-transport | **text** | 0.808 → 0.840 | 0.032 | 0.808 → 0.840 | 2.6x sr_airtime | up | 2 |
| `SF-catchup` | catch-up-hours | **text** | 0.778 → 0.809 | 0.032 | 0.778 → 0.809 | 9.2x advert_bytes | up | 3 |
| `TH-congestion-input` | congestion-input | **text** | 0.523 → 0.554 | 0.031 | 0.523 → 0.554 | 1.4x sr_airtime | up | 2 |
| `MS-router-late` | router-late-fraction | **held** | 0.927 → 0.948 | 0.021 | 0.799 → 0.811 | 1.3x bytes_on_air | down | 4 |
| `AD-badrouters` | role-placement | **held** | 0.892 → 0.911 | 0.019 | 0.694 → 0.706 | 1.5x sr_bytes | up | 3 |
| `DM-mode` | dm-mode | **text** | 0.763 → 0.781 | 0.018 | 0.763 → 0.781 | 1.3x sr_airtime | up | 3 |
| `SF-bucket-time` | time-bucket-s | **text** | 0.790 → 0.806 | 0.016 | 0.790 → 0.806 | 5.5x advert_bytes | up | 3 |
| `SF-sr-retries` | sr-retries | **text** | 0.804 → 0.819 | 0.015 | 0.804 → 0.819 | 1.1x sr_bytes | up | 4 |
| `AD-worst` | role-placement | **held** | 0.942 → 0.957 | 0.015 | 0.868 → 0.878 | 1.1x sr_bytes | up | 2 |
| `SF-window-size` | window-size | **held** | 0.936 → 0.950 | 0.014 | 0.800 → 0.810 | 5.2x advert_bytes | down | 3 |
| `SF-width` | short-id-bits | **text** | 0.801 → 0.815 | 0.014 | 0.801 → 0.815 | 3.1x advert_bytes | down | 4 |
| `SF-jitter-global` | advert-jitter-s | **held** | 0.941 → 0.955 | 0.014 | 0.805 → 0.809 | 1.2x sr_airtime | up | 4 |
| `SF-jitter-local` | advert-jitter-s | **held** | 0.941 → 0.955 | 0.014 | 0.805 → 0.809 | 1.2x sr_airtime | up | 4 |
| `SF-servers-flat` | servers | **text** | 0.808 → 0.821 | 0.013 | 0.808 → 0.821 | 7.7x sr_bytes | up | 4 |
| `SF-bucket-mode` | bucket-mode | **held** | 0.936 → 0.949 | 0.013 | 0.798 → 0.809 | 2.7x advert_bytes | down | 4 |
| `SF-servers-spread` | servers | **text** | 0.808 → 0.821 | 0.013 | 0.808 → 0.821 | 7.7x sr_bytes | up | 4 |
| `RT-hopassign` | hop-assign | **held** | 0.944 → 0.955 | 0.011 | 0.808 → 0.815 | 1.3x sr_airtime | up | 2 |
| `SF-servers-allrouters` | servers | **text** | 0.809 → 0.817 | 0.008 | 0.809 → 0.817 | 2.3x sr_bytes | down | 2 |
| `SF-capacity` | capacity | **held** | 0.939 → 0.946 | 0.007 | 0.806 → 0.810 | 5.3x advert_bytes | up | 5 |
| `SF-capacity-local` | capacity | **held** | 0.939 → 0.946 | 0.007 | 0.806 → 0.810 | 5.3x advert_bytes | up | 5 |
| `LD-diurnal` | diurnal | **text** | 0.808 → 0.815 | 0.007 | 0.808 → 0.815 | 1.2x advert_bytes | down | 3 |
| `FW-versions` | profile | **text** | 0.800 → 0.808 | 0.007 | 0.800 → 0.808 | 3.3x bytes_on_air | up | 5 |
| `PR-dmmode-cr` | dm-mode | **held** | 0.928 → 0.935 | 0.007 | 0.781 → 0.785 | 1.2x sr_airtime | up | 2 |
| `PR-crladder` | coding-rate-ladder | **held** | 0.928 → 0.934 | 0.006 | 0.781 → 0.785 | 1.1x sr_bytes | down | 2 |
| `SF-capacity-window` | capacity | **text** | 0.803 → 0.809 | 0.006 | 0.803 → 0.809 | 2.3x advert_bytes | down | 3 |
| `TH-congestion-mode` | congestion-mode | **text** | 0.920 → 0.925 | 0.005 | 0.920 → 0.925 | 1.1x sr_airtime | down | 2 |
| `RT-adopt` | no-adopt-hop-recommendation | **text** | 0.915 → 0.920 | 0.005 | 0.915 → 0.920 | 1.1x bytes_on_air | down | 2 |
| `FW-firmware` | profile | **text** | 0.803 → 0.808 | 0.005 | 0.803 → 0.808 | 3.1x bytes_on_air | up | 2 |
| `SF-resolve` | resolve | **held** | 0.939 → 0.944 | 0.004 | 0.804 → 0.808 | 5.7x advert_bytes | = | 3 |
| `SF-replay-order-broadcast` | replay-ordering | **text** | 0.835 → 0.840 | 0.004 | 0.835 → 0.840 | 1x sr_bytes | down | 2 |
| `SF-advert-transport` | advert-transport | **text** | 0.804 → 0.808 | 0.003 | 0.804 → 0.808 | 2.9x sr_airtime | down | 2 |
| `PR-repeats-busy` | extra-repeats | **held** | 0.992 → 0.994 | 0.002 | 0.920 → 0.920 | 1x sr_airtime | up | 2 |
| `SF-replay-order` | replay-ordering | **text** | 0.808 → 0.809 | 0.002 | 0.808 → 0.809 | 1.2x sr_bytes | up | 2 |
| `PR-repeats` | extra-repeats | **text** | 0.808 → 0.809 | 0.002 | 0.808 → 0.809 | 1.1x sr_bytes | up | 2 |
